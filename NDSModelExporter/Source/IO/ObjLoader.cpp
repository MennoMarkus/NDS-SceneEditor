...
bool ObjLoader::load(const Settings& settings, std::vector<Model>& o_models)
{
	// Get directory path
	std::string inputDir = "";
	size_t lastSlashPos = settings.inputFile.find_last_of("\\/");
	inputDir = (std::string::npos == lastSlashPos) ? "" : settings.inputFile.substr(0, lastSlashPos + 1);
	...
	bool success = tinyobj::LoadObj(&attributes, &shapes, &materials, &warningMessage, &errorMessage, settings.inputFile.c_str(), inputDir.c_str(), true, false);
	...
	for (int s = 0; s < shapes.size(); s++)
	{
		...
		for (int f = 0; f < shapes[s].mesh.indices.size() / 3; f++)
		{
			...
			else
			{
				for (int i = 0; i < 3; i++)
				{
					diffuseColor[0][i] = -1.f;
					diffuseColor[1][i] = -1.f;
					diffuseColor[2][i] = -1.f;
				}
